package cz.cvut.fit.pid_application.activity;
...
import cz.cvut.fit.pid_application.R;
...
// This contains the MapView in XML and needs to be called after the access token is configured.
        ...
        final Button button = findViewById(R.id.btn_);
...
// Initialize the mapboxMap view
        ...
        mapView.getMapAsync(new OnMapReadyCallback() {
            ...
            public void onMapReady(MapboxMap mapboxMap) {
                BusStopsActivity.this.mapboxMap = mapboxMap;
                mapboxMap.setStyle(Style.DARK, new Style.OnStyleLoaded() {
                    @Override
                    public void onStyleLoaded(@NonNull Style style) {
                        new LoadGeoJson(BusStopsActivity.this).execute();
                    }
                });
            }
        });
...
// using resources when the activities not in view.
    ...
    private static class LoadGeoJson extends AsyncTask<Void, Void, FeatureCollection> {
        ...
        private WeakReference<BusStopsActivity> weakReference;

        LoadGeoJson(BusStopsActivity activity) {
        ...
        try {
            BusStopsActivity activity = weakReference.get();
            if (activity != null) {
                InputStream inputStream = activity.getAssets().open("dejvicka.geojson");
